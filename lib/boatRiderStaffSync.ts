...
export async function syncBoatRiderToStaff(rider: BoatRiderLike): Promise<void> {
  try {
    ...
    const payload = {
      ...
      salary:
        rider.riderType === 'company'
          ? Number(rider.monthlySalaryLKR) || 0
          : Number(rider.contractPricePerRideLKR) || Number(rider.monthlySalaryLKR) || 0,
      employmentType: rider.riderType === 'company' ? 'full-time' : 'contract',
    ...
    // Log but never let sync failures break the rider save
    console.error('[boatRiderStaffSync] sync failed (non-fatal):', err);
  }
}

export const syncCompanyRiderToStaff = syncBoatRiderToStaff;
